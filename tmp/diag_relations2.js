...
async function check() {
  const { data: d1, error: e1 } = await supabase.from('study_groups').select('*, profiles!manager_id(*)').limit(1);
  console.log('sg error:', e1 ? e1.message : 'SUCCESS', d1);

  const { data: d2, error: e2 } = await supabase.from('help_requests').select('*, profiles!requester_id(*)').limit(1);
  console.log('hr error:', e2 ? e2.message : 'SUCCESS', d2);
}
